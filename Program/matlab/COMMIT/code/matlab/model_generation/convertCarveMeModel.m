...
if translate
    ...
    % reactions from rxnMetaNetXID
    for i=1:numel(model.rxns)
        if ~contains(model.rxns{i}, 'MNXR')
            new_rxn = regexp(model.rxnMetaNetXID{i}, 'MNXR[0-9]*', 'match');
            if ~isempty(new_rxn)
    ...
    % Metabolites from metMetaNetXID
    for i=1:numel(model.mets)
        if ~contains(model.mets{i}, 'MNXM')
            new_met = regexp(model.metMetaNetXID{i}, 'MNXM[0-9]*', 'match');
            if ~isempty(new_met)
